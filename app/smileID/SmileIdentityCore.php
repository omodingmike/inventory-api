...
//});
    ...
    class SmileIdentityCore
    {
         ...
         * @throws Exception
         */
        public function submit_job ( array $partner_params , array $image_details , array $id_info , $_options )
        {
            Utils ::validatePartnerParams( $partner_params );

            $job_type = $partner_params[ 'job_type' ];

            Utils ::validateIdParams( $id_info , $job_type );
            ...
            Utils ::validateImageParams( $image_details , $job_type , key_exists( 'use_enrolled_image' , $options ) && $options[ 'use_enrolled_image' ] );
            ...
            $code             = Utils ::array_value_by_key( 'code' , $response_body );
            if ( $code != '2202' ) {
                $message = Utils ::array_value_by_key( 'error' , $response_body );
                if ( !$message ) {
                    $message = Utils ::array_value_by_key( 'message' , $response_body );
                }
         ...
         * @throws Exception
         */
        private function submit_kyb_job ( array $partner_params , array $id_info ) : ResponseInterface
        {
            $signature_params = $this -> sig_class -> generate_signature();
            $data             = [
        ...
        private function generate_zip_file ( $response_body , $id_info , $images_info , $partner_params , $signature_params , $options ) : string
        {
            ...
            if ( $res === TRUE ) {
                ...
                foreach ( $images_info as $image ) {
                    if ( Utils ::endsWith( $image[ "image" ] , '.png' )
                        || Utils ::endsWith( $image[ "image" ] , '.jpg' )
                        || Utils ::endsWith( $image[ "image" ] , '.jpeg' ) ) {
         ...
         * @return array
         */
        private function configure_info_json ( $prep_upload_response_array , $id_info , $images , $partner_params , $signature_params , $options ) : array
        {
            $callback = $options[ 'optional_callback' ];
            $misc     = [
                "retry"           => "false" ,
         ...
         * @throws Exception
         */
        public function query_job_status ( array $partner_params , array $options ) : array
        {
            $signature_params = $this -> sig_class -> generate_signature();
         ...
         * @throws GuzzleException
         */
        public function get_job_status ( array $partner_params , array $options )
        {
